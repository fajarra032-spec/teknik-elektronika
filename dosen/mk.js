...
router.get('/', async (req, res) => {
  try {
    ...
    for (const doc of snapshot.docs) {
      ...
      // Hitung jumlah mahasiswa terdaftar aktif DI SEMESTER INI. Tanpa
      // filter semester, enrollment mahasiswa angkatan-angkatan
      // sebelumnya yang pernah ambil MK dengan mkId sama (status tidak
      // pernah "ditutup" saat semester berganti - lihat catatan di
      // routes/mahasiswa/elearning.js) ikut kehitung, jumlahnya jadi
      // menggelembung dan tidak mencerminkan kelas semester berjalan.
...
router.get('/:id/mahasiswa', async (req, res) => {
  try {
    ...
    // Ambil enrollment aktif untuk MK ini, DI SEMESTER INI - lihat
    // catatan di GET '/' di atas soal kenapa filter semester ini perlu
    // (kalau tidak, mahasiswa dari semester-semester lama yang pernah
    // ambil MK ini ikut nyampur di roster/daftar peserta).
    const enrollmentSnapshot = await db.collection('enrollment')
      .where('mkId', '==', mkId)
      .where('status', '==', 'active')
...
router.get('/:id', async (req, res) => {
  try {
    ...
    // ===== AMBIL DAFTAR MAHASISWA DARI ENROLLMENT (semester berjalan) =====
    const enrollmentSnapshot = await db.collection('enrollment')
