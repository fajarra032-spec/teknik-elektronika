/**
 * routes/dosen/buku.js
 * Dosen dapat mengunggah buku karya sendiri ke ELK Library. Buku yang
 * diunggah dosen berstatus 'pending' sampai disetujui admin (sama seperti
 * alur penelitian & pengabdian).
 */
...
async function getFolderBuku(dosenId) {
...
// Daftar buku milik dosen ini
router.get('/', async (req, res) => {
  try {
    const snapshot = await db.collection('buku')
      .where('dosenId', '==', req.dosen.id)
      .orderBy('createdAt', 'desc')
      .get();
    const list = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    res.render('dosen/karya/buku_index', {
      title: 'Buku Saya',
      list,
      dosen: req.dosen
    });
    ...
    console.error(err);
    res.status(500).render('error', { title: 'Error', message: 'Gagal memuat data buku' });
  }
});

// Form tambah
router.get('/tambah', (req, res) => {
  res.render('dosen/karya/buku_form', {
    title: 'Tambah Buku',
    data: null,
    dosen: req.dosen
  });
...
router.post('/tambah', upload.single('file'), async (req, res) => {
  try {
    const { judul, penerbit, isbn, tahun, deskripsi } = req.body;
    if (!judul) return res.status(400).send('Judul wajib diisi');
    let fileUrl = null, fileId = null;
    if (req.file) {
      const folderId = await getFolderBuku(req.dosen.id);
    ...
    await db.collection('buku').add({
      ...
      history: [{ action: 'submitted', timestamp: new Date().toISOString(), catatan: 'Pengajuan buku' }]
    });
    res.redirect('/dosen/buku');
  } catch (err) {
    console.error(err);
...
router.get('/:id', async (req, res) => {
  try {
    ...
    res.render('dosen/karya/buku_detail', { title: 'Detail Buku', data, dosen: req.dosen });
  } catch (err) {
    console.error(err);
...
router.post('/:id/hapus', async (req, res) => {
  try {
    const ref = db.collection('buku').doc(req.params.id);
