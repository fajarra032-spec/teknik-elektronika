...
async function getMataKuliahDiambil(userId) {
  try {
    // Filter juga by semester akademik SAAT INI - lihat penjelasan lengkap
    // di routes/mahasiswa/dashboard.js -> getMataKuliahDiambil (masalah
    // yang sama persis: enrollment.status tidak pernah "ditutup" saat
    // semester berganti, jadi tanpa filter ini MK + tugas dari semester
    // SEBELUMNYA tetap tampil terus setelah semester berpindah).
...
router.get('/', async (req, res) => {
  try {
    ...
    await Promise.all(mkList.map(async (mk) => {
      // Hitung jumlah mahasiswa terdaftar di MK ini, DI SEMESTER INI (query
      // count() sudah murah, dibilling sebagai 1 read terlepas dari jumlah
      // dokumen yang cocok)
